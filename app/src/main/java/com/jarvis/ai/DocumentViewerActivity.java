...
public class DocumentViewerActivity extends AppCompatActivity {
    ...
    private void loadSlides() {
        ...
        if (slides.isEmpty()) {
            // High quality executive multi-slide deck tailored to docTitle
            String title = (docTitle != null && !docTitle.isEmpty()) ? docTitle : "Executive Presentation";

            SlideData s1 = new SlideData();
            s1.title = title;
            s1.bullets.add("Strategic Architecture & Neural Reasoning Overview");
            s1.bullets.add("Prepared by H.E.N.R.Y. Document & Presentation Engine");
            s1.bullets.add("Confidential • Internal Review Edition");
            s1.notes = "Welcome team. Today we explore the primary pillars, deployment milestones, and strategic trajectory of this initiative.";
            slides.add(s1);

            SlideData s2 = new SlideData();
            s2.title = "Core Objectives & Scope";
            s2.bullets.add("Deliver scalable, autonomous intelligence with zero-latency response cycles.");
            s2.bullets.add("Unify multimodal telemetry, document generation, and planetary sensor analysis.");
            s2.bullets.add("Establish enterprise-grade reliability and seamless native workflow integration.");
            s2.notes = "Our core objective is end-to-end execution without friction or external dependency failures.";
            slides.add(s2);

            SlideData s3 = new SlideData();
            s3.title = "Technical Architecture & Design";
            s3.bullets.add("Real-time reactive UI driven by asynchronous Android background pipelines.");
            s3.bullets.add("Native OpenXML / PDF rendering pipelines for on-device document generation.");
            s3.bullets.add("Resilient dual-layer mapping and sensory radar telemetry integration.");
            s3.notes = "Highlight the modular architecture that enables offline autonomy and instant recovery.";
            slides.add(s3);

            SlideData s4 = new SlideData();
            s4.title = "Operational Telemetry & Performance";
            s4.bullets.add("Sub-100ms UI interaction responsiveness with hardware acceleration.");
            s4.bullets.add("Dynamic conversational reasoning and contextual intent resolution.");
            s4.bullets.add("Real-time error tolerance with graceful progressive fallback.");
            s4.notes = "Review the benchmark telemetry showing consistent stability across all operational modes.";
            slides.add(s4);

            SlideData s5 = new SlideData();
            s5.title = "Roadmap & Action Items";
            s5.bullets.add("Phase 1: Full native deployment and system verification.");
            s5.bullets.add("Phase 2: Broadened sensory radar integration and real-time telemetry.");
            s5.bullets.add("Phase 3: Continuous cognitive optimization and proactive task execution.");
            s5.notes = "Conclude with clear ownership and the immediate action items for the upcoming sprint.";
            slides.add(s5);
        }
    ...
    private void parsePptxFile(File file) {
        slides.clear();
        java.util.TreeMap<Integer, SlideData> slideMap = new java.util.TreeMap<>();
        try (ZipInputStream zis = new ZipInputStream(new FileInputStream(file))) {
            ZipEntry entry;
            while ((entry = zis.getNextEntry()) != null) {
                String name = entry.getName();
                if (name.contains("ppt/slides/slide") && name.endsWith(".xml") && !name.contains("_rels")) {
                    ...
                    if (slide != null) {
                        int num = extractSlideNumber(name);
                        slideMap.put(num, slide);
                    }
                }
            }
        } catch (Exception ignored) {}
        ...
        if (!slideMap.isEmpty()) {
            slides.addAll(slideMap.values());
        }
    }

    private int extractSlideNumber(String name) {
        try {
            java.util.regex.Matcher m = java.util.regex.Pattern.compile("slide(\\d+)\\.xml").matcher(name);
            if (m.find()) {
    ...
    private SlideData parseSlideXml(String xml) {
        ...
        if (s.notes.isEmpty()) {
            s.notes = "Presenter Talking Points: Deliver the core strategic message with confidence and clarity.";
        }
        return s;
    }
